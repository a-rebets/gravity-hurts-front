...
import { Panel, DOMHelper } from 'rsuite';

const { addClass, hasClass, removeClass } = DOMHelper;

class StoryReaction extends Component {
	state = {
		animating: 0,
		currentInd: -1,
	};

	starRefs = Array(5).fill(createRef());
	navRef = createRef();

	starClickHandler = (e) => {
		e.preventDefault();
		const clicked = e.currentTarget;
		const clickedInd = parseInt(clicked.id);
		if (!this.state.animating) {
			let last;
			if (this.state.currentInd !== -1) {
				last = this.starRefs[this.state.currentInd].current;
			}
			this.starRefs.forEach((ref, ind) => {
				ref.current.classList.toggle('active', clickedInd > ind);
			});
			this.setState({ animating: clickedInd > this.state.currentInd ? 1 : -1 });
			this.navRef.current.style.setProperty('--x', `${clicked.offsetLeft}px`);
			addClass(clicked, 'move-to');
			addClass(last ? last : clicked, 'move-from');
			setTimeout(() => {
				this.setState({ currentInd: clickedInd, animating: 0 });
				removeClass(clicked, 'move-to');
				if (last) {
					removeClass(last, 'move-from');
				}
			}, 800);
		}
	};
	...
	render() {
		const animate = this.state.animating;
		const animateClass = animate
			...
			: '';
		return (
			<Panel className='story-reaction' header='Как проходит твой день?'>
				<nav className={`rating ${animateClass}`} ref={this.navRef}>
					<ul>
						{Array.from(Array(5), (_, i) => (
							<li
								...
								id={i}
								className={
									i === this.state.currentInd && !animate ? 'current' : ''
								}
